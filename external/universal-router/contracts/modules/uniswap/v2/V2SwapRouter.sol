...
import {IUniswapV2Pair} from "@v2-core/IUniswapV2Pair.sol";
...
abstract contract V2SwapRouter is UniswapImmutables, Permit2Payments {
    ...
    function _v2Swap(address[] calldata path, address recipient, address pair) private {
        unchecked {
            ...
            uint256 penultimatePairIndex = path.length - 1 - 1;
            for (uint256 i; i < path.length - 1; i++) {
                (address input, address output) = (path[i], path[i + 1]);
                (uint256 amount0Out, uint256 amount1Out) = getOutputs(pair, input, token0);
