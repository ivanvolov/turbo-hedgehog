...
abstract contract ALMTestBase is Deployers {
    ...
    function alignOraclesAndPools(uint160 newSqrtPrice) public {
        uint256 poolPrice = TestLib.getPriceFromSqrtPriceX96(newSqrtPrice);

        uint256 oraclePrice = TestLib.getOraclePriceFromPoolPrice(poolPrice, isInvertedPool, int8(bDec) - int8(qDec));
        uint256 ratio = 10 ** uint256(int256(int8(bDec) - int8(qDec)) + 18);
        uint256 price = oraclePrice.mul(ratio);

        vm.mockCall(address(oracle), abi.encodeWithSelector(IOracle.price.selector), abi.encode(price));
        vm.mockCall(address(oracle), abi.encodeWithSelector(IOracle.poolPrice.selector), abi.encode(price, poolPrice));
    ...
    function oraclePriceW() internal view returns (uint256) {
        uint256 ratio = 10 ** uint256(int256(int8(bDec) - int8(qDec)) + 18);
        return oracle.price().div(oracle.ratio());
    }
